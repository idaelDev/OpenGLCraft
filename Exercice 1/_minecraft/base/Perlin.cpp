...
void Perlin::initBruit1D(int t, int p, int n) {
	...
	valeurs1D = (double*)malloc(sizeof(double) * (int)ceil(taille * pow((float)2, nombre_octaves1D - 1) / pas1D));

	srand(time(NULL));

	int i;

	for (i = 0; i < ceil(taille * pow((float)2, nombre_octaves1D - 1) / pas1D); i++)

		valeurs1D[i] = (double)rand() / RAND_MAX;

}

void Perlin::destroyBruit1D() {
...
double Perlin::bruit_coherent1D(double x, double persistance) {
...
void Perlin::initBruit2D(int l, int h, int p, int n) {
	...
	longueur_max = (int)ceil(longueur * pow((float)2, nombre_octaves2D - 1) / pas2D);

	int hauteur_max = (int)ceil(hauteur * pow((float)2, nombre_octaves2D - 1) / pas2D);
